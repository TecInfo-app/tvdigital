  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="font-geist text-xl font-bold text-gray-900">Terminais de Rede ({players.length})</h2>
        ...
        <button 
          ...
          className="border border-brand-outline-variant hover:bg-brand-surface-variant/40 text-gray-900 p-2.5 rounded-xl cursor-pointer transition-colors"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
        {players.map((player) => {
          ...
          return (
            <div 
              key={player.id} 
              className="glass-card p-6 rounded-2xl border border-gray-200 40 flex flex-col justify-between hover:border-blue-200 transition-all duration-300 shadow-lg group"
            >
              <div>
                {/* Header */}
                <div className="flex justify-between items-start gap-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-brand-surface-container rounded-xl border border-gray-200 20 flex items-center justify-center">
                    ...
                    <div>
                      <h3 className="font-geist font-bold text-gray-900 text-base">{player.name}</h3>
                ...
                {/* Playing Info block */}
                <div className="mt-5 p-3.5 bg-brand-surface-lowest/60 border border-gray-200 20 rounded-xl flex items-center gap-4">
                  <div className="w-16 h-10 rounded-lg bg-brand-surface-container overflow-hidden border border-gray-200 10 shrink-0 relative">
                    {currentMedia ? (
                  ...
                  {player.status !== 'offline' && (
                    <button 
                      ...
                      className="p-2 bg-blue-600-container text-brand-on-primary-container hover:scale-105 active:scale-95 transition-all rounded-lg cursor-pointer"
                ...
                {/* Metrics */}
                {player.status !== 'offline' ? (
                  <div className="grid grid-cols-2 gap-4 mt-5 pt-4 border-t border-gray-200 20">
                    <div>
                        ...
                        <div className="flex-1 h-1.5 bg-brand-surface-container rounded-full overflow-hidden">
                          <div 
                            className={`h-full rounded-full transition-all duration-500 ${
                              player.cpu > 80 ? 'bg-brand-tertiary' : 'bg-blue-600'
                            }`}
                            style={{ width: `${player.cpu}%` }}
                          ></div>
                        </div>
                        <span className="font-mono-data text-xs font-bold text-gray-900 w-7 text-right">{player.cpu}%</span>
                      </div>
                    </div>
                    <div>
                      <p className="text-[10px] text-brand-outline font-geist">Uso de Banda</p>
                      <p className="font-mono-data text-xs font-bold text-gray-900 mt-1">{player.bandwidth} Mbps</p>
                    </div>
                  </div>
                ) : (
                  <div className="mt-5 pt-4 border-t border-gray-200 20 text-center py-2">
              ...
              <div className="flex gap-2 mt-5">
                <button
                  ...
                  className="flex-1 py-2 bg-brand-surface-variant/40 hover:bg-brand-surface-variant/70 text-gray-900 rounded-xl font-bold text-xs cursor-pointer transition-colors disabled:opacity-30 disabled:pointer-events-none"
                >
