...
import { Search, Bell, MonitorPlay, Activity } from 'lucide-react';
...
export default function Header({
  ...
  return (
    <header className="flex justify-between items-center w-[calc(100%-328px)] h-16 ml-[312px] px-8 sticky top-4 z-40 bg-white/5 border border-white/15 rounded-2xl backdrop-blur-xl shadow-lg mt-4 mr-4">
      <div className="flex items-center gap-10">
        <h1 className="font-geist text-sm font-black text-white uppercase tracking-wider">
          FastPlayer Pro
        </h1>
        <div className="flex gap-6 h-full items-center">
          <button
            onClick={() => setActiveSubTab('health')}
            className={`font-semibold text-xs pb-1 border-b-2 transition-all cursor-pointer ${
              activeSubTab === 'health'
          ...
          <button
            ...
            className={`font-semibold text-xs pb-1 border-b-2 transition-all cursor-pointer flex items-center gap-1.5 ${
              activeSubTab === 'alerts'
                ? 'text-white border-white'
                : 'text-white/60 hover:text-white border-transparent'
            }`}
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="bg-white/5 border border-white/10 rounded-full pl-9 pr-4 py-1.5 text-xs text-white placeholder-white/40 focus:ring-1 focus:ring-white/30 focus:outline-none transition-all w-52 md:w-64"
          />
        </div>
        ...
        <div className="flex items-center gap-4 border-l border-white/10 pl-6 h-6">
          ...
          <button
            ...
            className="text-white/60 hover:text-white hover:bg-white/10 p-1.5 rounded-lg transition-all cursor-pointer flex items-center gap-1.5 text-xs font-semibold"
          >
          ...
          <div className="relative">
            <button
              ...
              className="text-white/60 hover:text-white hover:bg-white/10 p-1.5 rounded-lg transition-all cursor-pointer relative"
            >
            ...
            {showNotifications && (
              <div className="absolute right-0 mt-3 w-80 bg-slate-900/90 border border-white/15 rounded-xl shadow-2xl p-4 backdrop-blur-2xl z-50">
          ...
          <button
            onClick={onDeployAll}
            className="bg-white/10 hover:bg-white/20 border border-white/20 text-white px-4 py-1.5 rounded-lg font-bold text-xs hover:opacity-90 active:scale-[0.97] transition-all cursor-pointer shadow-md flex items-center gap-1.5"
          >
